...
const props = defineProps<{
    formateur: {
        id: number;
        nom: string;
        disciplines: { id: number; nom_discipline: string; formation_id: number | null }[];
    };
    disciplines: { id: number; nom_discipline: string }[];
    formations: { id: number; titre: string; disciplines: { id: number; nom_discipline: string }[] }[];
}>();

// Formation sélectionnée
const selectedFormation = ref('');

// Initialiser le formulaire avec la formation sélectionnée
const form = useForm({
    discipline_ids: [],
    formation_id: '',
});
    ...
    <MonLayout>
        <div class="container mx-auto px-4 py-6">
            <h1
                ...
                <i class="fas fa-user-tie mr-3 text-blue-500 dark:text-blue-400"></i>
            ...
            <form @submit.prevent="submit" class="max-w-3xl mx-auto bg-white dark:bg-gray-800 p-6 rounded-lg shadow-lg">
                ...
                <div class="mb-6">
                    <label for="formation" class="block text-sm font-medium text-gray-700 dark:text-gray-200 mb-2">
                ...
                <!-- Disciplines (filtrées par formation via table pivot) -->
                <div class="mb-6" v-if="selectedFormation">
                    ...
                    <div
                        ...
                        <label v-for="discipline in filteredDisciplines" :key="discipline.id"
                            ...
                            <span class="text-gray-900 dark:text-gray-100">{{ discipline.nom_discipline }}</span>
                        </label>
                        <p v-if="filteredDisciplines.length === 0" class="text-gray-500 dark:text-gray-400">
