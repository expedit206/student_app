...
const props = defineProps<{
    carnetData: {
        titre: string; // Titre de la formation ou classe
        ecole: string; // Nom de l'école
        apprenant: string; // Nom de l'apprenant
        ...
        disciplines: Array<{
            ...
            formateur: string;
            coefficient: number;
            note: string; // Note sur 20 (ex. "15/20")
            statut: string;
        }>;
        total_heures: number;
...
const printCarnet = () => {
    if (window.innerWidth >= 768) {
        ...
        alert('L’impression est disponible uniquement sur desktop.');
    }
};
</script>
            ...
            <header class="mb-8 animate-slide-in">
                <div class="bg-gray-800 p-4 sm:p-6 rounded-xl shadow-lg text-center">
                    ...
                    <button @click="printCarnet"
                        class="mt-4 bg-indigo-600 text-white px-4 py-2 rounded-lg shadow-md hover:bg-indigo-700 transition-all duration-300 flex items-center mx-auto">
                    ...
                    <table class="w-full text-left text-gray-200">
                        ...
                        <tbody>
                            <tr v-for="(discipline, index) in props.carnetData.disciplines" :key="discipline.id"
                                ...
                                <td class="p-4">{{ discipline.id }}</td>
                                <td class="p-4 font-medium">{{ discipline.titre }}</td>
                                <td class="p-4">{{ discipline.heures_hebdo }}h</td>
                                <td class="p-4">{{ discipline.heures_total }}h</td>
                                <td class="p-4">{{ discipline.formateur }}</td>
                                <td class="p-4">{{ discipline.coefficient }}</td>
                                <td class="p-4">{{ discipline.note[0] || 'N/A' }}</td>
                                <td class="p-4">
                                    <span
                                        :class="discipline.statut === 'Validé' ? 'text-green-400' : 'text-yellow-400'">
                                        {{ discipline.statut }}
                                    </span>
                                </td>
                            </tr>
                ...
                <div class="md:hidden space-y-4">
                    <div v-for="(discipline, index) in props.carnetData.disciplines" :key="discipline.id"
                        ...
                        <div class="grid grid-cols-2 gap-2 mt-2 text-sm text-gray-200">
                            ...
                            <p><i class="fas fa-star mr-1"></i>{{ discipline.notes || 'N/A' }} / 20</p>
                            <p>
                                <i class="fas fa-check-circle mr-1"></i>
                                <span :class="discipline.statut === 'Validé' ? 'text-green-400' : 'text-yellow-400'">
                                    {{ discipline.statut }}
                                </span>
                            </p>
                        </div>
...
/* Styles pour l'impression (Desktop uniquement) */
@media print {
